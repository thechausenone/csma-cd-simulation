...
class PersistentSimulator(Simulator):
    ...
    def run_single_iteration(self):
        ...
        while curr_simulation_time < self.duration:
            ...
            if curr_simulation_time - old_simulation_time < 0:
                print("we fucked up")
            old_simulation_time = curr_simulation_time
    ...
    def get_next_node(self):
        ...
        if curr_min_node is None:
            raise Exception("There are no nodes with packets available, simulation should terminate.")
    ...
    def handle_node_statuses(self, src_node, node_statuses):
        ...
        for node_status in node_statuses:
            status, dest_node = node_status
            if status == Status.COLLISION:
                collision_occured = True
                dest_node.increment_collisions()
                wait_time = generate_backoff_time(dest_node.num_collisions)
                if wait_time is None:
                    dest_node.remove_queue_head_time()
                    dest_node.reset_collisions()
                    self.num_dropped += 1
                else:    
                    dest_node.update_queue_times(wait_time + src_node.get_queue_head_time())
        ...
        if collision_occured:
            ...
            wait_time = generate_backoff_time(src_node.num_collisions)
            if wait_time is None:
                src_node.remove_queue_head_time()
                src_node.reset_collisions()
                self.num_dropped += 1
            else:    
                src_node.update_queue_times(wait_time + src_node.get_queue_head_time())
        else:
    ...
    def compute_metrics(self):
        ...
        throughput = total_num_successful * (self.packet_length / self.duration)
